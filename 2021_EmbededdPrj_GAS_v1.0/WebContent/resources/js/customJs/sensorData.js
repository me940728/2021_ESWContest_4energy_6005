...
// |   작성일     |   작성자    |                          내용                        |
// |------------------------------------------------------------------------------------
// | 2021.09.14   |  최별규     |  초안 작성
// | 2021.09.15   |  최별규     |  최종 완성(반드시!!프로미스나 async로 업데이트하기!!)
// -------------------------------------------------------------------------------------
...
'use strict'
   ...
   //---------------------------------------------몽고 데이터 불러오기(메인 컨트롤러)------------------------------------------
	...
	function initSensorData(changeFan)
	{
	    ...
	    $.ajax({
	        ...
	        success : function (data) { // 데이터 형식 : {"1" : data, "2" : data, "3" : data}
	            ...
	            //console.log("resFan : " + resFan);
	            //console.log(data); // 리턴되는 sensorJSON 로그확인
	        }
	    });
	    return resFan;
	}
    ...
    function initModal(data, limit)
    {
		...
		sensorArr.push(data["1"]); // => 1번 센서
		sensorArr.push(data["2"]); // => 2번 센서
		sensorArr.push(data["3"]); // => 3번 센서
	...
	{ 
    	if(num === 1) { // => 동작
    		$("3fanImg").attr("src", "/resources/img/fan.gif");
    	} else { // => 멈춤
    		$("3fanImg").attr("src", "/resources/img/fan.jpg");
    	}
    }